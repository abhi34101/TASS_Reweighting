...
class Reweight:
    ...
    def METAD_adiabatic(self, extT, colvar_file, outdir, mintozero, mtzcrd, mtzgsize):
        ...
        if self.conv==True:
            #loop run
            endpt=int((colvar.data.shape[0]/self.params["NWALKER"])/8)*self.params["NWALKER"]
            while endpt < colvar.data.shape[0]:
                ...
                endptout=(endpt/2000000)*self.params["NWALKER"]
                if len(self.params["GRID_PARAMS"])==1:
                    ...
                    cmd1="mv Ph1D.dat {}/Ph1D_{}.dat".format(outdir, endptout)
                    writeFE(self.pmf, np.array([cv_keys]), "FESout_1D.dat")
                    cmd2="mv FESout_1D.dat {}/FESout_1D_{}.dat".format(outdir, endptout)
                    os.system(cmd1)
                    os.system(cmd2)
                    ...
                    cmd1 = "mv Ph2D.dat {}/Ph2D_{}.dat".format(outdir, endptout)
                    writeFE(self.pmf, np.array([cv1_keys, cv2_keys]), 'FESout_2D.dat')
                    cmd2="mv FESout_2D.dat {}/FESout_2D_{}.dat".format(outdir, endptout)
                    os.system(cmd1)
                    os.system(cmd2)
                else:
            ...
            if len(self.params["GRID_PARAMS"])==2:
                ...
                print('N-dimensional data. No pmf will be written.')
                os.system(cmd1)
        return None
...
def arg_sanity_check(args):
    ...
    if args.mintozero and args.method=="METAMD":
        if not args.mtzgsize:
            print("--mtzgsize not used. Falling back to default gridsize of [5, 5]")
            args.mtzgsize=[5]
        else:
            args.mtzgsize=[int(args.mtzgsize)]
        if not args.mtzcoord:
            print("--mtzcoord not used. We will use the global minima instead.")
        else:
            coordstr=args.mtzcoord.split(",")
            args.mtzcoord=[int(i) for i in coordstr]
    else:
        print("--mintozero only implemented for method METAMD. Turning of mintozero flag.")
...
def main(args):
    ...
    if args.method in ["TASS", "WS"]:
        ...
        print(args.mintozero, args.mtzcoord, args.mtzgsize)
